...
const categories = ["All", "Rings", "Necklaces", "Earrings", "Bracelets", "Watches", "Collections", "Accessories"]
...
export default function GallerySection2() {
  ...
  const getRelatedImages = (currentImageIndex) => {
    const currentImage = galleryImages[currentImageIndex]
    if (!currentImage) return []

    return galleryImages
      .filter((img) => img.category === currentImage.category && galleryImages.indexOf(img) !== currentImageIndex)
      .slice(0, 6)
  }
  ...
  useEffect(() => {
    if (displayImages.length > 0) {
      setShimmerIndex(Math.floor(Math.random() * displayImages.length))
    }
  }, [displayImages])
  ...
  return (
    <div className="w-full px-4 md:px-10 py-4 sm:py-6">
      <div className="flex justify-center mt-4 mb-8">
        <div className="text-center max-w-4xl">
          ...
          >
            Discover our exquisite collection of handcrafted jewelry, from timeless classics to contemporary designs.
            Each piece tells a story of artistry, precision, and passion.
          </motion.p>
        </div>
      </div>
